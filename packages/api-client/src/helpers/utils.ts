...
export function arrayToTree<T extends HasParent>(nodes: T[]): RootNode<T> {
  ...
  for (const node of nodes) {
    mappedArr[node.id] = { ...(node as any), items: [] };
  }
    ...
    if (mappedArr.hasOwnProperty(id)) {
      ...
      if (!parentIsRoot) {
        if (mappedArr[parent.id]) {
          mappedArr[parent.id].items.push(mappedElem);
        } else {
          mappedArr[parent.id] = { items: [mappedElem] } as any;
        }
      } else {
  ...
  return { id: rootId, items: topLevelNodes };
}
